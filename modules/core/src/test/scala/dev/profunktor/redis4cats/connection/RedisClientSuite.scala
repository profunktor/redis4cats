...
class RedisClientSuite extends FunSuite {
  ...
  // quietPeriod=0, so building/releasing the resource needs no running Redis. Lettuce 7 removed
  // RedisURI.getPassword, so we resolve the credentials provider's Mono reflectively (avoids the
  // reactor JSR-305 annotation classfile-parse bug, scala/bug#7014). Works on Lettuce 6 and 7.
  private def passwordOf(uri: RedisURI): String = {
    ...
    Option(rc.getPassword).map(new String(_)).orNull
  }
