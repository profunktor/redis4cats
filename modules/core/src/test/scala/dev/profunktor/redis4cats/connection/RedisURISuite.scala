...
class RedisURISuite extends FunSuite {
  ...
  // Lettuce 7 removed RedisURI.getPassword/getUsername; credentials now live in a (lambda)
  // RedisCredentialsProvider whose only accessor returns a reactor Mono. We resolve it reflectively
  // (resolveCredentials().block()) so the Scala compiler never parses reactor's JSR-305 @Nullable
  // annotations (scala/bug#7014, fatal under -Werror). Version-robust across Lettuce 6 and 7.
  private def creds(uri: RedisURI): io.lettuce.core.RedisCredentials = {
    val provider = uri.underlying.getCredentialsProvider
    val mono =
      Class.forName("io.lettuce.core.RedisCredentialsProvider").getMethod("resolveCredentials").invoke(provider)
    Class
      .forName("reactor.core.publisher.Mono")
      .getMethod("block")
      .invoke(mono)
      .asInstanceOf[io.lettuce.core.RedisCredentials]
  }
