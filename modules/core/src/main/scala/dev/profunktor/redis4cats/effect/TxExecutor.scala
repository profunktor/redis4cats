...
private[redis4cats] object TxExecutor {
  def make[F[_]: Async]: Resource[F, TxExecutor[F]] =
    Resource
      .make(
        Sync[F].delay(
          Executors.newFixedThreadPool(1, TxThreadFactory).asInstanceOf[ThreadPoolExecutor]
        )
      ) { ec =>
        ...
        // misses a command that was actively executing (and forcibly cut off) at shutdown time. Waiting
        // briefly for the pool to actually terminate after the interrupt gives a direct signal instead:
        // a task that merely needed a moment to notice the interrupt and unwind terminates well within
        // the grace period, while one that's truly stuck (e.g. blocked on uninterruptible I/O) does not.
        Async[F]
          .blocking {
            val queued     = ec.shutdownNow()
            val terminated = ec.awaitTermination(500, TimeUnit.MILLISECONDS)
            (queued, terminated)
          }
          .flatMap { case (queued, terminated) =>
            new IllegalStateException(
              "There were outstanding tasks at time of shutdown of the Redis thread"
            ).raiseError[F, Unit]
