...
class SubscriberSuite extends IOSuite {
  ...
  test("messages published while the real subscribe call is still in flight are not dropped") {
    // Reproduces (in the absence of the fix) the message-drop window this design would otherwise reopen:
    // Subscribing carries its Topic from the moment it's created, before `commands.subscribe` is even called,
    // so onMessage always has somewhere to route a message regardless of whether the "subscribe" command
    // (held open here via `releaseSubscribe`) has resolved yet.
    //
    // Publishing and releasing the gate happens from a separate, concurrently-started fiber: subscribeAwait's
    // own resource acquisition blocks on `releaseSubscribe`, so completing it from inside the `.use` block
    // below would deadlock - that block can't start running until acquisition finishes.
    for {
      releaseSubscribe <- IO.deferred[Unit]
      map <- subscriptionMap(releaseSubscribe.get, IO.unit)
      _ <- (
             IO.sleep(50.millis) *> // give addSubscription a moment to install the Subscribing(topic) state
               map.onMessage(channel1, "early") *>
               releaseSubscribe.complete(())
           ).start
      result <- map
                  .subscribeAwait(channel1)
                  .flatMap(_.take(1).compile.toList.background)
                  .use(_.flatMap(_.embedError))
    } yield assertEquals(result, List("early"))
  }
  ...
  test("handle unsubscribe failure") {
    ...
    for {
      ...
      _ <- unsubscribe.attempt.map(attempt => assert(attempt.isLeft))
